...
class Stats:
    ...
    async def from_db(cur: aiomysql.Cursor, user_id: int, mode: Mode) -> "Stats":
        ...
        await cur.execute(
            ...
            "playcount_{m} playcount, total_hits_{m} total_hits, max_combo_{m} max_combo FROM {table} WHERE id = %s".format(
    ...
    async def get_rank(self) -> None:
        board = self.mode.leaderboard_str
    ...
    async def save(self, cur: aiomysql.Cursor) -> None:
        await cur.execute(
            ...
            "total_hits_{m} = %s, max_combo_{m} = %s WHERE id = %s".format(
                m=self.mode.db_prefix, table=self.mode.stats_table
            ),
            (
                self.ranked_score,
                self.total_score,
    ...
    async def recalc(self, cur: aiomysql.Cursor, score_pp: int = None) -> None:
        ...
        # if not self._recalc_pp or (score_pp and score_pp > self._recalc_pp):
        await cur.execute(
